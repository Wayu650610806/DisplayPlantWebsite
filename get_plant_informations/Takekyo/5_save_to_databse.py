...
def ocr_number(bgr):
    """
    (Robust Version 4) "Lean Tournament"
    ลดการเรียก Tesseract จาก 9 เหลือ 4 ครั้ง เพื่อความเร็วบน Server
    """
    ...
    def parse(t):
        ...
        s = m.group().replace(",", ".")
        try: return float(s), t
        except: return None, t

    # 1. กำหนด Configs (เหลือ 3 ตัวหลัก)
    cfg_6 = f"--oem 1 --psm 6 -c tessedit_char_whitelist=0123456789.-"
    cfg_7 = f"--oem 1 --psm 7 -c tessedit_char_whitelist=0123456789.-"
    cfg_13 = f"--oem 1 --psm 13 -c tessedit_char_whitelist=0123456789.-" # (ตัวแก้ "17")

    # 2. Kernel (สำหรับทำให้หนา)
    kernel = np.ones((2, 2), np.uint8)

    # 3. สร้างภาพ 4 เวอร์ชัน (ขั้นตอนนี้เร็ว)
    ...
    img_adapt_thin = cv2.adaptiveThreshold(g, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11, 2)
    img_adapt_thick = cv2.dilate(img_adapt_thin, kernel, iterations=1)
    
    # 4. 🔥 (NEW) รัน "การแข่งขันแบบลีน" (Test 4 แบบ)
    results = []

    # 1. (Otsu บาง - ตัวเต็งปกติ)
    results.append(parse(pytesseract.image_to_string(img_otsu_thin, config=cfg_7)))
    
    # 2. (Otsu หนา - ตัวแก้ "17")
    results.append(parse(pytesseract.image_to_string(img_otsu_thick, config=cfg_13)))
    
    # 3. (Adapt บาง - สำรองแสงเพี้ยน)
    results.append(parse(pytesseract.image_to_string(img_adapt_thin, config=cfg_7)))
    
    # 4. (Adapt หนา - สำรองแสงเพี้ยน + "17")
    results.append(parse(pytesseract.image_to_string(img_adapt_thick, config=cfg_13)))
    
    # (แถม) ลอง psm 6 กับ Otsu หนา เผื่อไว้
    results.append(parse(pytesseract.image_to_string(img_otsu_thick, config=cfg_6)))


    # 5. เลือกผู้ชนะ (เหมือนเดิม)
    valid_results = [r for r in results if r[0] is not None]

    if not valid_results:
        all_raw = sorted([r[1] for r in results], key=len, reverse=True)
        return (None, all_raw[0])
    
    # เลือกอันที่ Raw Text "ยาวที่สุด"
    valid_results.sort(key=lambda x: len(x[1]), reverse=True)
...
def show_images_in_folder(folder: str, output_folder: str, status_template_dir: str):
    ...
    # --- โหลด Metadata ---
    timestamp_json_path = os.path.join(output_folder, "timestamps_takekyo.json"); all_timestamps = {}
...
if __name__ == "__main__":
    ...
    status_template_dir = r"C:\Project\DisplayPlantWebsite\output_tab_images\status_loop" # แก้ Path ตามที่คุณใช้

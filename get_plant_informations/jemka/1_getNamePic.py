...
def split_manual_2_1(img):
    """
    แบ่งภาพออกเป็น 4 ส่วน: ~34%, ~34%, ~16%, ~16%
    (ปรับจาก 33, 33, 16, 16 เนื่องจากรวมได้ 98%)
    ส่งค่ากลับเป็น list ของ (x, y, w, h)
    """
    h, w = img.shape[:2]
    
    # คำนวณจุดแบ่ง 3 จุด
    split_x_1 = (w * 34) // 100  # 34%
    split_x_2 = (w * 68) // 100  # 34% + 34% = 68%
    split_x_3 = (w * 84) // 100  # 68% + 16% = 84%
    ...
    # กล่อง 4: 84% - 100% (เก็บส่วนที่เหลือทั้งหมด ~16%)
    x4 = split_x_3
    w4 = w - split_x_3
...
if __name__ == "__main__":
    ...
    input_folder = r"C:\Project\DisplayPlantWebsite\uploaded_images\tv1\1" 
